...
sys.path.append("../pycatch-master/pcrasterModules/")
...
class CatchmentModel(DynamicModel, MonteCarloModel):
    ...
    def initial(self):
        ...
        self.samples = generalfunctions.samplingScheme(self.clone, nrSampleLocs, fractionShortDistance,
    ...
    def dynamic(self):
        ...
        if calculateStats:
            if cfg.variances:
                # temporal
                dist, gamma = generalfunctions.experimentalVariogramValuesInTime(stackOfMapsAsListVariable,
                                                                                 list(boundVector))
                numpy.savetxt(generateNameST('sfT', self.currentSampleNumber(), self.currentTimeStep()), numpy.array(gamma))
                # spatial
                dist, gamma = generalfunctions.experimentalVariogramValues(stackOfMapsAsListVariable, boundVector, 1, 1,
                                                                           'test', 2.0)
        ...
        totQ = ifthen(self.clone,
                      maptotal(ifthenelse(outflowPoints, self.d_runoffAccuthreshold.RunoffCubicMetrePerHour, scalar(0))))
        ...
        if calculateStats:
            ...
            # mean
            generalfunctions.reportLocationsAsNumpyArray(self.aLocation, totQ, 'qA', self.currentSampleNumber(),
    ...
    def postmcloop(self):
        import generalfunctions
        names = ['gA', 'bioA', 'regA', 'sfA', 'qA' ,'gpA', 'grA', 'grNA', 'depA', 'weaA', 'creA']
        for name in names:
