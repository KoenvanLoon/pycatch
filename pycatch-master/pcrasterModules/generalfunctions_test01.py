...
def printCellValue(self, mapVariable, variableNameToPrint, unit, row, column):
    ...
    print(variableNameToPrint + ' (' + unit + ') at row ' + str(row) + ', column: ' + str(column) + ' is: ' + str(
        cellValue))
...
def stackOfMapsToRowAsArray(stackOfMapsAsList, currentTime):
    timeOfFirstMap = currentTime - len(stackOfMapsAsList) + 1
    t = timeOfFirstMap
    stackOfMapsAsRowsList = []
    for map in stackOfMapsAsList:
        mapArray = mapToRowAsArray(map)
        mapAsRowArrayWithTime = addTimeRowToMapAsRowArray(mapArray, t)
        stackOfMapsAsRowsList.append(mapAsRowArrayWithTime)
        t = t + 1
...
def semvar(firstMap, secondMap):
    nrPairs = getCellValue(
        pcr.cover(pcr.maptotal(pcr.scalar(pcr.pcrand(pcr.defined(firstMap), pcr.defined(secondMap)))), 0), 1, 1)
...
def experimentalVariogramValuesInTime(stackOfMapsAsList, bounds):
    ...
    # print(nrPairsOfLags) # Put out of order for the time being - KL
    ...
    for i in range(0, nMaps):
        for j in range(i + 1, nMaps):
            ...
            for k, bound in enumerate(bounds):
                if dist < bound:
                    nrPairsOfLags[k] += nrPairs
                    sumOfSquaredDiffOfLags[k] += sumOfSquaredDiff
                    sumOfDists[k] += nrPairs * float(dist)
                    break
    semvarList = map(truedivKoen, sumOfSquaredDiffOfLags, nrPairsOfLags)
...
def semvarOfStackOfMapsInSpace(stackOfMapsAsList, lagX, lagY):
    ...
    for theMap in stackOfMapsAsList:
        ...
        nrPairsTot += nrPairs
        sumOfSquaredDiffTot += sumOfSquaredDiff
...
def samplingScheme(clone, nrSamples, fractionShortDistance, separationDistance, nrCellsToRight, nrCellsToTop):
    ...
    randomSampleNumbers = pcr.ordinal(
        pcr.ifthen(samplesAtGrid, pcr.order(pcr.uniform(pcr.ifthen(samplesAtGrid, pcr.boolean(1))))))
    ...
    for sample in range(1, int(getCellValue(pcr.mapmaximum(randomSampleNumbers), 1, 1)) + 1):
        ...
        if getCellValue(totalSamples, 1, 1) > (nrSamples - 1.0 + 0.1):
            break
    sampleNumbers = pcr.ordinal(
        pcr.ifthen(alreadyCovered, pcr.order(pcr.uniqueid(pcr.ifthen(alreadyCovered, pcr.boolean(1))))))
    # shift the cells to centre
    centreCol = pcr.maptotal(colnumber) / nrCells(clone)
    centreRow = pcr.maptotal(rownumber) / nrCells(clone)
    centreColSamples = pcr.maptotal(pcr.ifthen(alreadyCovered, colnumber)) / pcr.maptotal(
        pcr.ifthen(alreadyCovered, pcr.scalar(1)))
    centreRowSamples = pcr.maptotal(pcr.ifthen(alreadyCovered, rownumber)) / pcr.maptotal(
        pcr.ifthen(alreadyCovered, pcr.scalar(1)))
    sampleNumbersShifted = pcr.shift(sampleNumbers, centreRow - centreRowSamples - nrCellsToTop,
                                     centreColSamples - centreCol - nrCellsToRight)
    return pcr.cover(sampleNumbersShifted, 0)


def samplingSchemeRandomShift(clone, nrSamples, fractionShortDistance, separationDistance, maxNrCellsToRight,
                              maxNrCellsToTop):
    ...
    sampleNumbersShifted = samplingScheme(clone, nrSamples, fractionShortDistance, separationDistance, shiftRight,
                                          shiftTop)
    return sampleNumbersShifted


def samplingSchemeSubset(clone, nrSamples, fractionShortDistance, separationDistance, nrCellsToRight, nrCellsToTop,
                         nrSamplesRemove, uniformMap,
                         realNrSamples):
...
def samplingSchemeRandomShiftSubset(clone, nrSamples, fractionShortDistance, separationDistance, maxNrCellsToRight,
                                    maxNrCellsToTop, nrSamplesRemove,
                                    uniformMap, realNrSamples):
    uniformRight = pcr.mapuniform()
    shiftRight = pcr.roundoff((uniformRight - 0.5) * maxNrCellsToRight)
    uniformTop = pcr.mapuniform()
    shiftTop = pcr.roundoff((uniformTop - 0.5) * maxNrCellsToTop)
    sampleNumbersShifted = samplingSchemeSubset(clone, nrSamples, fractionShortDistance, separationDistance, shiftRight,
                                                shiftTop, nrSamplesRemove, uniformMap,
                                                realNrSamples)
    return sampleNumbersShifted
...
def reportMemberVariablesOfAClassForSuspend(classInstance, currentTimeStep, currentSampleNumber):
    ...
    for name in sorted(b):
        if type(b[name]) == PCRaster._PCRaster.Field:
            fileName = createFileNameToReportAVariableForSuspend(classInstance, names[i], currentTimeStep,
                                                                 currentSampleNumber)
